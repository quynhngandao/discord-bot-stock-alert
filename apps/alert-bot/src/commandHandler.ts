...
import { db, alerts, symbols } from '@discord-slack-alert-bot/db';
import { runScan, MANUAL_SCAN_LIMIT } from './scanOrchestrator.js';
import { isMarketClosed, isMarketOpen } from '../../utils/marketCalendar.js';
import { DISCLAIMER } from '@discord-slack-alert-bot/alerts';
...
async function handleScan(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  try {
    ...
    await interaction.editReply(
      `Scan complete (${count + 1}/${MAX_MANUAL_SCANS_PER_DAY} today) — ${results.length} candidates, ${passed} passed filters.`,
    );
  } catch (err) {
...
async function handleWatchlist(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  const rows = await db.select().from(alerts).where(gte(alerts.sentAt, todayStart())).limit(10);
  ...
  const lines = rows.map(
    (a) =>
      `**${a.ticker}** — score ${a.score ?? 'N/A'} · ${a.alertType} · ${new Date(a.sentAt).toLocaleTimeString('en-US', { timeZone: 'America/Chicago' })} CST`,
  );

  const embed = new EmbedBuilder().setTitle("Today's Alerts").setColor(0xffd700).setDescription(lines.join('\n')).setTimestamp();
