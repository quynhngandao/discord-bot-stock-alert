...
import { getCachedFundamentals, setCachedFundamentals } from '../infrastructure/db/fundamentalsCacheService.js';
...
import { assignRelativeStrengthRanks, computeIbdMetrics, computeMinerviniMetrics, computeRsScore, computeSpyReturns } from '../scanner/indicators.js';
...
import { processResults } from '@discord-stock-alert-bot/alerts/';
...
async function loadActiveTickers(limit: number): Promise<string[]> {
  const rows = await db.select().from(symbols).where(eq(symbols.isActive, true)).orderBy(asc(symbols.id));
...
async function fetchPricesForAll(tickers: string[], provider: 'tiingo' | 'yahoo' = 'tiingo'): Promise<Map<string, HistoricalPrice[]>> {
  const delayMs = provider === 'yahoo' ? 200 : 500;
  const results = await withRateLimit(
    tickers.map((ticker) => (): Promise<[string, HistoricalPrice[]] | null> => {
      if (provider === 'yahoo') {
...
async function fetchFundamentalsForAll(tickers: string[]): Promise<{ statementsMap: Map<string, IncomeStatement[]>; roeMap: Map<string, number> }> {
...
function buildScanResult(
  ...
  const dataCompletenessScore = totalRules === 0 ? 100 : Math.round(((totalRules - missingFields.length) / totalRules) * 100);

  const beatsSpy63d = minervini.return63d !== null && minervini.spy63dReturn !== null && minervini.return63d > minervini.spy63dReturn;

  const beatsSpy21d = minervini.return21d !== null && minervini.spy21dReturn !== null && minervini.return21d > minervini.spy21dReturn;
...
export async function runScan(limit = SCHEDULED_SCAN_LIMIT, provider: 'tiingo' | 'yahoo' = 'tiingo'): Promise<StockScanResult[]> {
  ...
  const minerviniPassing = minerviniMetrics.filter((m) => evaluateMinervini(m).passesAvailableRules);
