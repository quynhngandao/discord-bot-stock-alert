...
import { DISCLAIMER } from '@discord-stock-alert-bot/alerts';
...
export async function sendNewsAlert(ticker: string, headline: string, source: string, url: string): Promise<void> {
  const channel = await discordClient.channels.fetch(process.env.ALERT_CHANNEL_ID);
  ...
  const embed = new EmbedBuilder()
    ...
    .setColor(0xf4a261)
    .addFields({ name: 'Source', value: source, inline: true }, { name: 'Link', value: url, inline: false })
...
export async function sendScanAlert(result: StockScanResult, score: number, priority: 'WATCHLIST' | 'HIGH PRIORITY'): Promise<void> {
  const formatGrowth = (v: number | null) => (v === null ? 'N/A' : `${v >= 0 ? '+' : ''}${v.toFixed(1)}%`);

  const rsVsSpy = result.beatsSpy63d ? (result.beatsSpy21d ? 'Outperforming SPY on 63d and 21d' : 'Outperforming SPY on 63d') : 'Underperforming SPY';
