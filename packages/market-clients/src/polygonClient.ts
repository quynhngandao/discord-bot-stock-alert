...
function computeTtmRoe(results: PolygonFinancialResult[]): number | null {
  ...
  const netIncomes = recent.map((r) => r.financials?.income_statement?.net_income_loss?.value ?? null);
  if (netIncomes.some((v) => v === null)) return null;
  const ttmNetIncome = (netIncomes as number[]).reduce((a, b) => a + b, 0);

  const latestEquity =
    results[0]?.financials?.balance_sheet?.equity_attributable_to_parent?.value ?? results[0]?.financials?.balance_sheet?.equity?.value ?? null;
...
export async function fetchFinancials(ticker: string, limit = 8): Promise<{ statements: IncomeStatement[]; ttmRoe: number | null }> {
  const data = await get<{ results?: PolygonFinancialResult[]; status: string }>('/vX/reference/financials', {
    ticker,
    timeframe: 'quarterly',
    limit: String(limit),
    sort: 'filing_date',
