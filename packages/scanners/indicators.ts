import type { HistoricalPrice, IncomeStatement, IbdApproxMetrics, MinerviniMetrics } from '@discord-stock-alert-bot/types';
...
export function computeMinerviniMetrics(symbol: string, prices: HistoricalPrice[]): MinerviniMetrics | null {
  ...
  if (sma50 === null || sma150 === null || sma200 === null || sma150OneMonthAgo === null || sma200OneMonthAgo === null) {
    return null;
  }
...
export function assignRelativeStrengthRanks(metrics: MinerviniMetrics[], rsScoreMap: Map<string, number>): void {
...
export function computeSpyReturns(spyPrices: HistoricalPrice[]): { return63d: number | null; return21d: number | null } {
...
function computeYoYGrowth(latest: IncomeStatement, all: IncomeStatement[]): { epsGrowth: number | null; revenueGrowth: number | null } {
  ...
  const epsGrowth = latest.eps !== null && prior.eps !== null && prior.eps !== 0 ? ((latest.eps - prior.eps) / Math.abs(prior.eps)) * 100 : null;
...
export function computeIbdMetrics(
  ...
  const quarterly = statements.filter(
    (s: IncomeStatement): boolean => s.period === 'Q1' || s.period === 'Q2' || s.period === 'Q3' || s.period === 'Q4',
  );
  ...
  return {
    ...
    epsAcceleration: latestGrowth?.epsGrowth != null && prevGrowth?.epsGrowth != null ? latestGrowth.epsGrowth > prevGrowth.epsGrowth : null,
    revenueAcceleration:
      latestGrowth?.revenueGrowth != null && prevGrowth?.revenueGrowth != null ? latestGrowth.revenueGrowth > prevGrowth.revenueGrowth : null,
  };
}
