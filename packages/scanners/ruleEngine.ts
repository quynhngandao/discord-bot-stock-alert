...
function rule(name: string, value: number | null, threshold: number | null, check: (v: number, t: number) => boolean): RuleResult {
...
function buildRuleSetResult(rules: RuleResult[]): RuleSetResult {
  ...
  const dataCompletenessScore = rules.length === 0 ? 100 : Math.round((nonMissing.length / rules.length) * 100);
...
export function evaluateMinervini(m: MinerviniMetrics): RuleSetResult {
  ...
  const rules: RuleResult[] = [
    ...(cfg.requireCloseAboveSma50 ? [rule('close > sma50', m.close, m.sma50, (v, t) => v > t)] : []),
