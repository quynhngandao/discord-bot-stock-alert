...
export function evaluateMinervini(m: MinerviniMetrics): RuleSetResult {
  ...
  const rules: RuleResult[] = [
    ...
    rule(
      ...
      cfg.minPercentAboveLow52Week,
      (v, t) => v >= t
    ),
    rule(
      "% from 52w high",
      m.percentFromHigh52Week,
      cfg.maxPercentFromHigh52Week,
      (v, t) => v <= t
    ),
...
export function evaluateIbd(ibd: IbdApproxMetrics): RuleSetResult {
  ...
  const rules: RuleResult[] = [
    rule(
      "EPS growth latest Q",
      ibd.epsGrowthLatestQuarter,
      cfg.minEpsGrowthLatestQuarter,
      (v, t) => v >= t
    ),
    rule(
      "EPS growth prev Q",
      ibd.epsGrowthPreviousQuarter,
      cfg.minEpsGrowthPreviousQuarter,
      (v, t) => v >= t
    ),
    rule(
      "Revenue growth latest Q",
      ibd.revenueGrowthLatestQuarter,
      cfg.minRevenueGrowthLatestQuarter,
      (v, t) => v >= t
    ),
    rule(
      "Revenue growth prev Q",
      ibd.revenueGrowthPreviousQuarter,
