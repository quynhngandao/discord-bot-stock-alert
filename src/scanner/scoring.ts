...
export function computeScore(
  ...
  _ibdResult: RuleSetResult,
  metrics: MinerviniMetrics
): ScoringBreakdown {
  // Trend (50): 10 pts each for 5 core Minervini criteria
  const trendRules = [
    "close > sma50", "close > sma150", "close > sma200",
    "sma150 rising", "sma200 rising",
    "sma50 > sma150", "sma50 > sma200",
    "% above 52w low",
    "% from 52w high",
  ];
  const trendPassed = minerviniResult.rules
    .filter((r) => trendRules.includes(r.name) && r.passedStrict)
    .length;
  ...
  const beats63d =
    metrics.return63d !== null &&
    metrics.spy63dReturn !== null &&
    metrics.return63d > metrics.spy63dReturn;
  const beats21d =
    metrics.return21d !== null &&
    metrics.spy21dReturn !== null &&
    metrics.return21d > metrics.spy21dReturn;
